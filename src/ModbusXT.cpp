...
void Modbus::process_F1_F2()
{
  ...
  unsigned char no_of_registers = _packet->data / 32;
...
uint8_t Modbus::construct_F15()
{
  ...
  uint8_t no_of_registers = _packet->data / 32;
 ...
 * @comment: none
 */
void Modbus::configure(Packet* packets, uint16_t total_packets, int32_t* register_array)
{
 ...
 * @comment: none
 */
void Modbus::configure_manual(Packet* packets, uint16_t total_packets, int32_t* register_array)
{
	_packet_array = packets;
	_register_array = register_array;
	_total_packets = total_packets;
